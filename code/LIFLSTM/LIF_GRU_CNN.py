"""
IF-GRU(CNN) in pytorch. data = DVS-Gestures
...
from module.LIFRNN_module import LIFConvGRU,LIFPooling,LIFGRU
...
def main():
    ...
    for dt in m:
        ...
        # 模型保存路径
        modelPath = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'results'
        modelNames = 'gesture_LIF-GRU(CNN)' + str(dt) + 'ms' + '.t7'
        # 运行记录保存路径
        recordPath = os.path.dirname(os.path.abspath(__file__)) + os.sep + 'record'
        recordNames = 'gesture_LIF-GRU(CNN)' + str(dt) + 'ms.csv'
        # dataset路径
