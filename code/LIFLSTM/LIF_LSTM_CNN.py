"""
LIF-LSTM(CNN) in pytorch. data = DVS-Gestures
...
from module.LIFRNN_module import LIFConvLSTM, LIFLSTM, LIFPooling
...
def main():
    ...
    for dt in m:
        ...
        print('LIF-LSTM(CNN)-DVS-Gesture:dt=', dt, 'ms')
        ...
        modelNames = 'gesture_LIF-LSTM(CNN)' + str(dt) + 'ms' + '.t7'
        # 运行记录保存路径
        recordPath = os.path.dirname(
            os.path.abspath(__file__)) + os.sep + 'record'
        recordNames = 'gesture_LIF-LSTM(CNN)' + str(dt) + 'ms.csv'
        # dataset路径
