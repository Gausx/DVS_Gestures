"""
SNNs(MLP) in pytorch. data = DVS-Gesture
...
def main():
    ...
    for dt in m:
        ...
        print('SNNs(MLP)-DVS-Gesture:dt=', dt, 'ms')
        ...
        modelNames = 'gesture_snn(MLP)' + str(dt) + 'ms' + '.t7'
        # 运行记录保存路径
        recordPath = os.path.dirname(
            os.path.abspath(__file__)) + os.sep + 'record'
        recordNames = 'gesture_snn(MLP)' + str(dt) + 'ms.csv'
        # dataset路径
        ...
        # fc layer
        cfg_fc = [in_channels * im_height * im_width, 512, target_size]

        class Net(nn.Module):
            def __init__(self):
                super(Net, self).__init__()
                self.fc1 = nn.Linear(cfg_fc[0], cfg_fc[1])
                self.fc2 = nn.Linear(cfg_fc[1], cfg_fc[-1])
        ...
        # {'Total': 1054731, 'Trainable': 1054731}
